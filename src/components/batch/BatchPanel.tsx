...
export default function BatchPanel() {
  ...
  const stopAll = useCallback(() => {
    abortRefs.current.forEach((c) => c.abort()); abortRefs.current.clear();
    tasks.forEach((task) => {
      if (['loading', 'analyzing', 'reviewing'].includes(task.status)) {
        updateTask(task.id, {
          status: task.status === 'reviewing' ? 'review_error' : 'error',
          error: '用户取消',
        });
      }
    });
    setProcessing(false); setReviewing(false);
  }, [tasks, updateTask, setProcessing, setReviewing]);
  ...
  const startReview = useCallback(async () => {
    ...
    setProcessing(true); setReviewing(true);
  ...
  const handleDrop = async (e: React.DragEvent) => { e.preventDefault(); e.stopPropagation(); setIsDragOver(false); dragCounter.current = 0; const file = e.dataTransfer.files[0]; if (file?.type.startsWith('image/')) await setSourceImage(file); };
